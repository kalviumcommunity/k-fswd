...
const saltView = document.getElementById('sha256-salt')
...
// a function to generate sha256 hash of the given string along the salt
...
async function getSHA256Hash() {
	let cached = retrieve('sha256', SALT)
	if (cached) {
		return cached
	}

	cached = await sha256(getRandomArbitrary(MIN, MAX), SALT)
	store('sha256', cached)
	return cached
}

async function main() {
	saltView.innerHTML = `Use the salt: '${SALT}'`
...
async function test() {
	...
	const sha256HashView = document.getElementById('sha256-hash')
	const hasedPin = await sha256(pin, SALT)
